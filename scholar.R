...
scholar = function(google.id='lhc97roAAAAJ', max.authors=3, years.since=2000, order.by='ayear'){
  ret = list() # start with blank output
  
  # 0) verify scholar ID and flag problems
  fail = FALSE; problem='' # flag for failure
  if(nchar(google.id) != 12){fail=T; problem='Google Scholar ID should be 12 characters'}
  if(fail==T){
    ret$name = problem
  ...
  # a) select person
  if(fail==F){
    bio = get_profile(id=google.id) # get basics
    name = bio$name
    affiliation = bio$affiliation
    h = bio$h_index # h-index
    
    # b) select works
    papers = get_publications(id=google.id) # get works
    ...
    papers$Title = as.character(papers$Title)
    papers$Journal = as.character(papers$Journal)
    papers$Year = as.numeric(papers$Year) # looks better as character? changed to year for sorting
